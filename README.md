...
src/
  ...
  hotness.ts         — расстояние до Универмага, оценка "горячести", формат сообщения
...
Для локальной сквозной проверки скопируй `.dev.vars.example` в
`.dev.vars` и подставь тестовые значения. `.dev.vars` и `.env` исключены
из Git; реальные секреты нельзя коммитить.
