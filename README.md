...
- **Кнопки.** После `/start` внизу чата появляется постоянная панель:
  ...
  количество комнат (`1` / `2` / `3` / `4+`, можно выбрать несколько) и
  ценовой диапазон (`до 200$` / `200–400$` / `400–600$` / `600$+` /
  `Любая цена`, выбор один из). Выбранное отмечается ✅, сообщение
  обновляется на месте при каждом тапе (без спама новыми сообщениями).
