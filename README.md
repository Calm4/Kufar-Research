...
src/
  ...
  telegram-webhook.ts — обработка команд от Telegram (/start, /price, /rooms, ...)
...
оно проходит фильтр (`src/filters.ts`). Команды боту:

- `/price 100-300` — присылать только объявления в диапазоне 100–300$.
  `/price off` — снять фильтр по цене.
- `/rooms 1,2` — присылать только 1- и 2-комнатные. `/rooms off` — снять.
...
Без единого фильтра — как раньше, приходит всё.
