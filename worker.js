...
export default {
  async scheduled(event, env, ctx) {
    // NOTE: self-fetching this worker's own public URL from here was tried
    // as a workaround and rejected by the Cloudflare runtime itself
    // (error 1042 — loop protection), before ever reaching Kufar. Back to
    // calling runMonitor() directly.
    ctx.waitUntil(
      runMonitor(env)
        .then((result) => {
          console.log(
